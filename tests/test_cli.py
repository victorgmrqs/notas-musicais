...
def test_escala_cli_dev_retornar_0_ao_stdout():
    result = runner.invoke(app, ['escala'])
    assert result.exit_code == 0


@mark.parametrize('nota', ['C', 'D', 'E', 'F', 'G', 'A', 'B'])
def test_escala_cli_dev_conter_as_notas_na_resposta(nota):
    result = runner.invoke(app, ['escala'])
...
def test_escala_cli_dev_conter_as_notas_na_resposta_de_re(nota):
...
def test_escala_cli_dev_conter_todos_os_graus(grau):
    result = runner.invoke(app, ['escala', 'F'])
...
def test_acorde_cli_dev_conter_as_notas_na_resposta(nota):
...
def test_acorde_cli_dev_conter_todos_os_graus(grau):
